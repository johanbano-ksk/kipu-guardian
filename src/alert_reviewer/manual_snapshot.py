...
def filter_snapshot(
    ...
    *,
    ...
    return accepted, len(projected), build_country_summary(
        [row.get("country") for row in rows],
        accepted,
    )
...
def filter_occurrence_snapshot(
    ...
    *,
    ...
    ai_selector: Callable[[list[dict[str, Any]]], set[int]] | None = None,
) -> tuple[list[dict[str, Any]], int, int, list[dict[str, str | int]]]:
    ...
    for record in records:
        ...
        if publication_timestamp.astimezone(business_timezone).date() != day:
            continue
        source_count += 1
        received_countries.append(_occurrence_country(record))
        if (
    ...
    return (
        ...
        len(candidates),
        source_count,
        build_country_summary(received_countries, accepted),
    )
...
def build_country_summary(
    ...
    """Aggregate source and accepted alerts without exposing rejected payloads."""

    received = Counter(_country_name(value) for value in received_countries)
    accepted = Counter(_country_name(alert.get("country")) for alert in accepted_alerts)
    return [
        {
            "country": country,
            "received_count": received.get(country, 0),
            "accepted_count": accepted.get(country, 0),
        }
        for country in sorted(received.keys() | accepted.keys())
    ]


def _occurrence_country(record: dict[str, Any]) -> Any:
    raw_payload = record.get("raw_payload")
    if isinstance(raw_payload, dict):
        return raw_payload.get("country")
